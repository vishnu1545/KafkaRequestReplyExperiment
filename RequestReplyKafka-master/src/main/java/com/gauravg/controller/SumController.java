...
public class SumController {
	...
	public Model sum() throws InterruptedException, ExecutionException {
		// create producer record
		Model mo=new Model();
		mo.setFirstNumber(22);
		mo.setSecondNumber(85);
		ProducerRecord<String, Model> record = new ProducerRecord<String, Model>(requestTopic,mo );
		...
		//print all headers
		sendResult.getProducerRecord().headers().forEach(header -> System.err.println(header.key() + ":" + header.value().toString()));
		
		// get consumer record
		ConsumerRecord<String, Model> consumerRecord = sendAndReceive.get();
		// return consumer value
		
		consumerRecord.headers().forEach(header -> System.err.println(header.key() + ":" + header.value().toString()));
